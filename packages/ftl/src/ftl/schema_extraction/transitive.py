...
class TransitiveExtractor(ast.NodeVisitor):
    ...
    @staticmethod
    def convert_ast_annotation_to_type_hint(annotation_node: ast.AST) -> Optional[Type[Any]]:
    ...
    def visit_ClassDef(self, node):
        if self.context.must_extract(self.module_name, node.name):
            ...
            for class_node in node.body:
                if isinstance(class_node, ast.AnnAssign):  # Annotated assignment (field)
                    field_name = class_node.target.id if isinstance(class_node.target, ast.Name) else None
                    if field_name and class_node.annotation:
                        type_hint = self.convert_ast_annotation_to_type_hint(class_node.annotation)
                        if type_hint:
            ...
            data = schemapb.Data(
                pos=schemapb.Position(filename=self.file_path, line=lineno, column=col_offset),
                name=node.name,
                fields=fields,
                export=export
            )
